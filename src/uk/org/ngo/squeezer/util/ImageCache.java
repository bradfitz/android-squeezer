...
public class ImageCache {
    ...
    // Control access to the disk
    private final Object mDiskAccessMonitor = new Object();
    ...
    public Bitmap getBitmapFromDiskCache(String data) {
        ...
        if (mICSDiskCache != null) {
            try {
                ...
                if (snapshot != null) {
                    // If disk access is paused then wait. Will be woken
                    // up by .notifyAll() in setPauseDiskCache().
                    synchronized (mDiskAccessMonitor) {
                        while (mPauseDiskAccess) {
                            try {
                                mDiskAccessMonitor.wait();
    ...
    public void setPauseDiskCache(boolean pause) {
        ...
        // Wake up threads that had paused
        if (!pause) {
            synchronized (mDiskAccessMonitor) {
                mDiskAccessMonitor.notifyAll();
            }
        }
    }
