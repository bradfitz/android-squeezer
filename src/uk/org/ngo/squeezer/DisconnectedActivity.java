 ...
 * <p>
 * Provide a UI for connecting to the configured server, launch SqueezerHomeActivity when
 * the user connects.
 */
public class DisconnectedActivity extends SqueezerBaseActivity {
     ...
     * Show this activity.
     * <p>
     * Flags are set to clear the previous activities, as trying to go back while disconnected
     * makes no sense.
     * <p>
     * The pending transition is overridden to animate the activity in place, rather than having
     * it appear to move in from off-screen.
     *
     * @param activity
     */
    ...
    public void onUserInitiatesConnect(View view) {
        NowPlayingFragment fragment = (NowPlayingFragment) getSupportFragmentManager().findFragmentById(R.id.now_playing_fragment);
    ...
    private final IServiceCallback serviceCallback = new IServiceCallback.Stub() {
        ...
        public void onConnectionChanged(final boolean isConnected, final boolean postConnect,
            ...
            if (isConnected) {
                ...
                // no prior activity to go to, so launch SqueezerHomeActivity, with flags to
                // clear other activities so hitting "back" won't show this activity again.
                getUIThreadHandler().post(new Runnable() {
                    @Override
                    public void run() {
                        final Intent intent = new Intent(DisconnectedActivity.this, SqueezerHomeActivity.class)
                                .addFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK)
                                .addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
